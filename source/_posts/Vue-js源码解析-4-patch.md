...
### 组件的两个vnode
1. 占位符vnode：vm.$vnode 只有组件实例才有。在 `_render` 过程中赋值
![](https://cdn.liujiefront.com/images/algorithm/fi64p.png)
2. 渲染vnode：vm._vnode 可以直接映射成真实DOM。在 `_update` 过程中赋值
...
* 对于 App 组件来说，他的占位符vnode 和渲染vnode 实际上是一种父子关系
...
```
（上一节的`createComponent`是定义在`src/core/vdom/create-component.js`，目的是返回组件 vnode；而这里的`createComponent`是根据vnode完成组件DOM的插入）
...
回顾[上一节](https://blog.liujiefront.com/2020/08/18/Vue-js%E6%BA%90%E7%A0%81%E8%A7%A3%E6%9E%90-3-createComponent/)，在执行 `createComponent` 函数的时候会调用 `installComponentHooks` 函数给 vnode.data.hook 安装四个钩子函数。
...
1. 创建 options 对象
   ...
   * _parentVnode 表示当前组件的vnode，也就是占位符vnode,`_render`过程中赋值
   * parent 表示当前激活的组件实例，也就是 根Vue实例
...
在完成组件的整个 patch 过程后，最后执行 `insert(parentElm, vnode.elm, refElm)` 完成组件的 DOM 插入，如果组件 patch 过程中又创建了子组件，那么DOM 的插入顺序是**先子后父**。`parentElm`最后会是body
